/**
 * Created by cai on 2014/9/21 16:09.
 *
...
Ext.define('app.view.module.form.BaseForm', {
    ...
    initComponent: function() {
        ...
        var groups = this.formScheme.tf_schemeGroups,
            hasTab = false;
        var hasInTabPanel = false;      // 是否有嵌在页面里的tab
        var inTablPanel;
        ...
        if (hasTab) {
            ...
            Ext.each(groups, function(group) {
                if (group.tf_displayMode == 'intabpanel') {
                    ...
                    Ext.apply(inTablPanel, me.getOtherSetting(group.tf_otherSetting));
                    me.items.push(inTablPanel);
                } else if (group.tf_displayMode == 'intab') {
                    var t = me.createFiledSetOrSubModule(group);
                    t.title = group.tf_formGroupName;
                    inTablPanel.items.push(t);
                } else {
    ...
    setRecordId: function(id) {
        ...
        this.module.model.load(id, {
            success: function(record, operation, success) {
                // success中的record中返回的raw数据，是字符串，没有经过decode，要自己转成对象
