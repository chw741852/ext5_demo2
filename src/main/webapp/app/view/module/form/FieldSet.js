/**
 * Created by cai on 2014/9/21 16:45.
 *
 * 生成form中的一个fieldSet的类
 */
Ext.define('app.view.module.form.FieldSet', {
    extend: 'Ext.form.FieldSet',
    alias: 'widget.formfiledset',

    requires: ['app.view.module.factory.FieldContainerFactory', 'app.view.module.factory.FormFieldFactory'],
    ...
    config: {
        ...
        schemeGroup: undefined,     // 定义了此filedSet的属性以及下面需要加的字段
        numCols: undefined,
        formtype: undefined
    },
    ...
    initComponent: function() {
        ...
        var containers = [];        // 要计算一下有多少个container，如果col=2，那么两个换行，或者指定换行
        var hiddens = [];           // 隐藏字段
        var container = [];
        var c = 0;
        ...
        for (var i in this.schemeGroup.tf_groupFields) {
            ...
            var fieldDefine = this.getViewModel().getFieldDefine(field.tf_fieldId);
        ...
        for (var i in containers) {
            this.items.push(app.view.module.factory.FieldContainerFactory.getContainer(containers[i], this.getViewModel(), this.formtype));
        }
        ...
        for (var i in hiddens) {
            ...
            var f = app.view.module.factory.FormFieldFactory.getField(fieldDefine, field, this.formtype);
            this.items.push(f);
        }
